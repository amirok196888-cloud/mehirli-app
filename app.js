...
$$('[data-quick-pricing]').forEach(button=>button.onclick=()=>{const mode=button.dataset.quickPricing;$('#priceFloor').dataset.manualOverride='false';$('#proQuotedPrice').dataset.manualOverride='false';if(mode==='hourly')$('#proLaborHours').value=1;setPricingMode(mode,true);$$('[data-quick-pricing]').forEach(b=>b.classList.toggle('active',b===button));toast('התמחור מולא לפי ההגדרות שלך — אפשר לשנות כל סכום')});
...
async function openNewProJob(){
  ...
  $('#proTravelCost').value=s.default_travel_cost||0;$('#proLaborHours').value=1;$('#proMaterialsCost').value=0;$('#proAssistantCost').value=0;$('#proDepositAmount').value=0;$('#proDiscountType').value='none';$('#proDiscountValue').value=0;$('#proDiscountValue').disabled=true;$('#priceFloor').value='';$('#priceFloor').dataset.manualOverride='false';$('#proQuotedPrice').value='';$('#proQuotedPrice').dataset.manualOverride='false';const valid=new Date();valid.setDate(valid.getDate()+14);$('#proQuoteValidUntil').value=valid.toISOString().slice(0,10);$('#smartAnalysis').classList.add('hidden');setPricingMode(s.default_pricing_mode||'hourly',true);const restored=restoreProJobDraft();show('#proJobFormView');if(restored)toast('הטיוטה הקודמת שוחזרה')
}
function proJobDraftKey(){return state.user?.id?`mehirli_pro_job_draft_v1:${state.user.id}`:''}
function saveProJobDraft(){const key=proJobDraftKey(),form=$('#proJobForm');if(!key||!form)return;const fields={};[...form.elements].forEach(el=>{if(el.id&&['INPUT','TEXTAREA','SELECT'].includes(el.tagName)&&el.type!=='file')fields[el.id]=el.type==='checkbox'?el.checked:el.value});try{localStorage.setItem(key,JSON.stringify({fields,quoteItems:state.quoteItems,savedAt:new Date().toISOString()}))}catch{}}
function restoreProJobDraft(){const key=proJobDraftKey();if(!key)return false;let draft=null;try{draft=JSON.parse(localStorage.getItem(key)||'null')}catch{}if(!draft?.fields)return false;Object.entries(draft.fields).forEach(([id,value])=>{const el=document.getElementById(id);if(!el)return;if(el.type==='checkbox')el.checked=Boolean(value);else el.value=value});state.quoteItems=Array.isArray(draft.quoteItems)?draft.quoteItems:[];renderQuoteItemsEditor();setTrade($('#proJobTrade').value||'handyman');setPricingMode($('#proPricingMode').value||'hourly',false);calculateProPrice(false);return true}
...
$('#proWorkspaceBtn').onclick=openProWorkspace;$('#homeNewJobBtn').onclick=openNewProJob;$('#newProJobBtn').onclick=openNewProJob;$('#sampleQuoteBtn').onclick=()=>window.open('./quote-preview-v61.jpg','_blank','noopener');$('#proJobStatusFilter').onchange=renderProJobCards;
