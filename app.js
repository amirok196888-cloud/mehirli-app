...
$$('#proJobForm [data-job-step]').forEach(button=>button.onclick=()=>setJobStep(Number(button.dataset.jobStep)));
$$('#proJobForm [data-next-step]').forEach(button=>button.onclick=()=>{const panel=button.closest('[data-job-panel]'),invalid=[...panel.querySelectorAll('input[required],textarea[required],select[required]')].find(input=>!input.checkValidity());if(invalid){invalid.reportValidity();return}setJobStep(Number(button.dataset.nextStep))});
$('#proJobForm').addEventListener('invalid',event=>{const panel=event.target.closest('[data-job-panel]');if(panel)setJobStep(Number(panel.dataset.jobPanel))},true);
