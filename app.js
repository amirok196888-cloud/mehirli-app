...
async function openNewProJob(skipDraft=false){
  ...
  $('#proTravelCost').value=s.default_travel_cost||0;$('#proLaborHours').value=1;$('#proMaterialsCost').value=0;$('#proAssistantCost').value=0;$('#proDepositAmount').value=0;$('#proDiscountType').value='none';$('#proDiscountValue').value=0;$('#proDiscountValue').disabled=true;$('#priceFloor').value='';$('#priceFloor').dataset.manualOverride='false';$('#proQuotedPrice').value='';$('#proQuotedPrice').dataset.manualOverride='false';const valid=new Date();valid.setDate(valid.getDate()+14);$('#proQuoteValidUntil').value=valid.toISOString().slice(0,10);$('#smartAnalysis').classList.add('hidden');setPricingMode(s.default_pricing_mode||'hourly',true);const restored=skipDraft!==true&&restoreProJobDraft();setJobStep(0);show('#proJobFormView');if(restored)toast('הטיוטה הקודמת שוחזרה')
}
...
async function openQuoteEditor(id){
  if(!requireServiceAccess())return;
  const {data:job,error}=await db.from('pro_jobs').select('*').eq('id',id).eq('professional_id',state.user.id).single();
  if(error||!job){toast('לא ניתן לפתוח את ההצעה לעריכה');return}
  const {data:items,error:itemsError}=await db.from('pro_job_items').select('*').eq('job_id',id).eq('professional_id',state.user.id).order('sort_order');
  if(itemsError){toast('לא ניתן לטעון את סעיפי ההצעה');return}
  await openNewProJob(true);
  state.selectedProJob={...job,items:items||[]};
  if(quoteCanEdit(job))state.editingProJob=job;else state.quoteRevisionSource=job;
  setTrade(ANALYSIS_RULES[job.trade]?job.trade:'handyman');
  const values={proCustomerSelect:job.customer_id,proCustomerName:job.customer_name,proCustomerPhone:job.customer_phone,proCustomerCity:job.city,proJobType:job.job_type,proJobDescription:job.description,proScheduledAt:dateTimeInputValue(job.scheduled_at),proPricingMode:job.pricing_mode||'hourly',proBasePrice:job.base_price,proLaborHours:job.labor_hours,proMaterialsCost:job.materials_cost,proTravelCost:job.travel_cost,proAssistantCost:job.assistant_cost,proDepositAmount:job.deposit_amount,proDiscountType:job.discount_type||'none',proDiscountValue:job.discount_value||0,priceFloor:job.price_floor,proQuotedPrice:job.quoted_price,proQuoteScope:job.quote_scope,proQuoteValidUntil:job.quote_valid_until,proWarrantyText:job.warranty_text};
  Object.entries(values).forEach(([key,value])=>{$('#'+key).value=value??''});
  // A single auto-generated total line must not override recalculation from hours/materials.
  const automaticLine=items?.length===1&&items[0].description===job.description&&Number(items[0].quantity)===1&&Number(items[0].unit_price)===Number(Number(job.discount_amount)>0?job.subtotal:job.quoted_price);
  state.quoteItems=automaticLine?[]:(items||[]).map(item=>({...item}));renderQuoteItemsEditor();
  $('#priceFloor').dataset.manualOverride='false';$('#proQuotedPrice').dataset.manualOverride='true';setPricingMode(job.pricing_mode||'hourly',false);
  $('#priceFloor').value=job.price_floor;$('#priceFloor').dataset.manualOverride=String(Number(job.price_floor)!==Number($('#priceFloor').dataset.calculatedValue));
  const pricing=calculateProPrice(false);$('#proQuotedPrice').dataset.manualOverride=String(Number(job.quoted_price)!==pricing.calculatedTotal);
...
$('#proJobForm').onsubmit=async e=>{
  ...
  if(state.quoteRevisionSource)row.quote_terms=state.quoteRevisionSource.quote_terms||'';
  if(state.editingProJob){
    const editing=state.editingProJob;row.quote_terms=editing.quote_terms||'';
    const items=state.quoteItems.filter(i=>String(i.description||'').trim()).map((i,index)=>({description:String(i.description).trim(),quantity:Number(i.quantity)||1,unit_price:Number(i.unit_price)||0,estimated_cost:Number(i.estimated_cost)||0,sort_order:index}));
    if(!items.length)items.push({description:row.description,quantity:1,unit_price:pricing.discountAmount>0?pricing.subtotal:pricing.total,estimated_cost:row.materials_cost,sort_order:0});
    const button=$('#proJobSubmitBtn');button.disabled=true;
    try{
...
function compactJobDetail(){
  const box=$('#jobDetailContent');
  const groups=[['.quote-send-callout','💬 שליחת הצעה ב־WhatsApp'],['.analysis-display','📋 שאלות ללקוח וציוד להכנה']];
  const fold=(node,title)=>{if(!node)return;const details=document.createElement('details');details.className='job-tool-fold';const summary=document.createElement('summary');summary.textContent=title;node.before(details);details.append(summary,node)};
  groups.forEach(([selector,title])=>fold(box.querySelector(selector),title));
  [...box.children].filter(el=>el.matches('div.card')).forEach(node=>{if(node.querySelector('[data-job-action="quote-pdf"]'))fold(node,'📄 הצעה, עריכה ו־PDF');else if(node.querySelector('#detailReminderAt'))fold(node,'🔔 תזכורת למעקב');else if(node.querySelector('#jobStatusSelect'))fold(node,'📌 עדכון מצב העבודה')});
}
async function renderProJobDetail(){
  ...
  $('#jobDetailContent').append(paymentPanel);paymentPanel.querySelector('[data-config-payment]').onclick=loadProSettingsForm;
