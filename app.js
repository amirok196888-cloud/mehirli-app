...
$('#proServiceTemplate').onchange=e=>{const t=serviceTemplatesForTrade($('#proJobTrade').value).find(x=>x.id===e.target.value);if(!t)return;$('#proQuotedPrice').dataset.manualOverride='false';renderJobTypeOptions(t.trade,t.job_type);$('#proJobDescription').value=t.name;$('#proLaborHours').value=t.default_hours||1;$('#proMaterialsCost').value=t.default_materials_cost||0;$('#proQuoteScope').value=t.quote_scope||t.name;setPricingMode(t.pricing_mode||'fixed',false);$('#proBasePrice').value=Number(t.base_price)||0;state.quoteItems=[];const pricing=calculateProPrice(true);state.quoteItems=[{description:t.name,quantity:1,unit_price:pricing.recommended,estimated_cost:Number(t.default_materials_cost)||0}];renderQuoteItemsEditor();calculateProPrice(true)};
