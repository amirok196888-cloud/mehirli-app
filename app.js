...
function analyticsAttribution(){
  const params=new URLSearchParams(location.search);let source=params.get('utm_source')||params.get('source')||'';
  if(!source&&document.referrer){try{source=new URL(document.referrer).hostname.replace(/^www\./,'')}catch{}}
  return {source:source||'direct',campaign:params.get('utm_campaign')||''}
}
...
const BUILTIN_SERVICES={
  handyman:[['תליית טלוויזיה','tv','fixed',350,2],['הרכבת רהיט','furniture','hourly',180,2],['חצי יום תיקונים','general','half_day',700,4],['יום עבודה היינדמן','general','full_day',1300,8]],
...
async function fillProfile(){const {data}=await db.from('business_profiles').select('*').eq('user_id',state.user.id).maybeSingle();state.businessProfile=data||null;$('#bizName').value=data?.business_name||'';$('#bizLegalName').value=data?.legal_name||'';$('#bizNumber').value=data?.business_number||'';$('#bizEmail').value=data?.business_email||'';$('#bizAddress').value=data?.business_address||'';$('#bizCategory').value=data?.specialties?.[0]||'היינדמן';$('#bizAbout').value=data?.description||'';$('#bizArea').value=(data?.service_areas||[]).join(', ');$('#bizPhone').value=data?.business_phone||'';renderBusinessLogoEditor()}
...
function serviceTemplatesForTrade(trade){const built=(BUILTIN_SERVICES[trade]||[]).map((x,i)=>({id:`builtin:${trade}:${i}`,trade,name:x[0],job_type:x[1],pricing_mode:x[2],base_price:x[3],default_hours:x[4],default_materials_cost:0,quote_scope:''}));return [...built,...state.proServices.filter(s=>s.trade===trade)]}
...
function setTrade(trade){
  $('#proJobTrade').value=trade;$$('.trade-choice').forEach(b=>b.classList.toggle('active',b.dataset.trade===trade));renderJobTypeOptions(trade);renderServiceTemplateOptions(trade);
}
...
async function openNewProJob(){
  ...
  $('#proTravelCost').value=s.default_travel_cost||0;$('#proLaborHours').value=1;$('#proMaterialsCost').value=0;$('#proAssistantCost').value=0;$('#proDepositAmount').value=0;$('#proDiscountType').value='none';$('#proDiscountValue').value=0;$('#proDiscountValue').disabled=true;const valid=new Date();valid.setDate(valid.getDate()+14);$('#proQuoteValidUntil').value=valid.toISOString().slice(0,10);$('#smartAnalysis').classList.add('hidden');setPricingMode(s.default_pricing_mode||'hourly',true);show('#proJobFormView')
}
...
function calendarEntries(){const jobs=state.proJobs.filter(j=>j.scheduled_at&&!['cancelled','paid'].includes(j.status)).map(j=>({id:`job:${j.id}`,date:j.scheduled_at,title:j.customer_name,body:j.description,kind:'job'})),reminders=state.proReminders.map(r=>({id:r.id,date:r.remind_at,title:r.message,body:'תזכורת',kind:'reminder'}));return [...jobs,...reminders].sort((a,b)=>new Date(a.date)-new Date(b.date))}
function renderCalendar(){const box=$('#calendarList'),rows=calendarEntries();box.innerHTML=rows.length?rows.map(x=>{const d=new Date(x.date);return `<div class="timeline-item ${d<Date.now()?'due':''}"><div class="timeline-date"><strong>${d.toLocaleDateString('he-IL',{day:'2-digit',month:'2-digit'})}</strong><small>${d.toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})}</small></div><div><h3>${x.kind==='job'?'🧰':'🔔'} ${esc(x.title)}</h3><p>${esc(x.body)}</p></div>${x.kind==='reminder'?`<button class="ghost tiny" data-done-reminder="${x.id}">בוצע</button>`:''}</div>`}).join(''):'<div class="card empty-state"><span>📅</span><h3>היומן פנוי</h3><p>מועד עבודה ותזכורת שנשמרים בתיק יופיעו כאן.</p></div>';box.querySelectorAll('[data-done-reminder]').forEach(b=>b.onclick=async()=>{const {error}=await db.from('pro_reminders').update({is_done:true}).eq('id',b.dataset.doneReminder).eq('professional_id',state.user.id);if(error){toast(error.message);return}await loadProReminders();renderCalendar();toast('התזכורת הושלמה')})}
async function openCalendar(){if(!requireServiceAccess())return;await Promise.all([loadProJobs(),loadProReminders()]);renderCalendar();show('#calendarView')}
...
$('#proServiceTemplate').onchange=e=>{const t=serviceTemplatesForTrade($('#proJobTrade').value).find(x=>x.id===e.target.value);if(!t)return;renderJobTypeOptions(t.trade,t.job_type);$('#proJobDescription').value=t.name;$('#proLaborHours').value=t.default_hours||1;$('#proMaterialsCost').value=t.default_materials_cost||0;$('#proQuoteScope').value=t.quote_scope||t.name;setPricingMode(t.pricing_mode||'fixed',false);$('#proBasePrice').value=Number(t.base_price)||0;state.quoteItems=[];const pricing=calculateProPrice(true);state.quoteItems=[{description:t.name,quantity:1,unit_price:pricing.recommended,estimated_cost:Number(t.default_materials_cost)||0}];renderQuoteItemsEditor();calculateProPrice(true)};
...
function storedQuoteMessage(job,pdfUrl){return `🟠 מחירלי | הצעת מחיר\n\nשלום ${job.customer_name}, הכנתי עבורך הצעת מחיר עבור: ${job.description}\n\nמחיר: ${money(job.quoted_price)}${Number(job.deposit_amount)>0?`\nמקדמה: ${money(job.deposit_amount)}`:''}\n\nפתיחת הצעת המחיר כ־PDF:\n${pdfUrl}\n\nלאישור ההצעה:\n${quoteUrl(job)}\n\nנשלח באמצעות מחירלי`}
...
$('#proSettingsForm').onsubmit=async e=>{
  e.preventDefault();if(!requireServiceAccess())return;const enteredPaymentLink=$('#settingsPaymentLink').value.trim(),paymentLink=safePaymentUrl(enteredPaymentLink),selectedProvider=$('#settingsPaymentProvider').value;if(enteredPaymentLink&&!paymentLink){toast('מטעמי אבטחה קישור התשלום חייב להתחיל ב־https://');return}if(selectedProvider&&!paymentLink){toast('בחרת חברת סליקה — עכשיו צריך להדביק את קישור התשלום שלה');return}const paymentProvider=paymentLink?(selectedProvider||'other'):null;const row={professional_id:state.user.id,trade:$('#settingsTrade').value,default_hourly_rate:numberValue('#settingsHourlyRate'),default_travel_cost:numberValue('#settingsTravelCost'),half_day_price:numberValue('#settingsHalfDayPrice'),full_day_price:numberValue('#settingsFullDayPrice'),half_day_hours:numberValue('#settingsHalfDayHours'),full_day_hours:numberValue('#settingsFullDayHours'),overtime_rate:numberValue('#settingsOvertimeRate'),time_rounding_minutes:numberValue('#settingsTimeRounding'),overhead_percent:numberValue('#settingsOverhead'),risk_percent:numberValue('#settingsRisk'),payment_provider:paymentProvider,payment_link:paymentLink||null,electrician_license_number:$('#settingsLicenseNumber').value.trim()||null,electrician_license_expiry:$('#settingsLicenseExpiry').value||null,quote_terms:$('#settingsQuoteTerms').value.trim()};
...
async function loadAdmin(){
  ...
  const [{data:summary,error:se},{data:jobs,error:je},{data:businesses,error:be},{data:billing,error:bse},{data:marketing,error:me}]=await Promise.all([
    db.rpc('admin_professional_summary_v33'),db.rpc('admin_list_pro_jobs'),db.rpc('admin_list_businesses_v33'),rpcWithFallback('admin_get_billing_settings_v38','admin_get_billing_settings_v33'),db.rpc('admin_marketing_summary_v40')
  ]);
  ...
  const funnel=marketing||{};$('#adminUniqueVisitors').textContent=funnel.unique_visitors||0;$('#adminVisitors30d').textContent=`30 יום: ${funnel.unique_visitors_30d||0}`;$('#adminTrialSignups').textContent=funnel.trial_signups||0;$('#adminAppInstalls').textContent=funnel.installs||0;$('#adminPayingCustomers').textContent=funnel.paying_customers||0;$('#adminVisitTrialConversion').textContent=`${Number(funnel.visitor_to_trial_percent||0).toLocaleString('he-IL')}%`;$('#adminTrialPaidConversion').textContent=`${Number(funnel.trial_to_paid_percent||0).toLocaleString('he-IL')}%`;
  const sources=Array.isArray(funnel.top_sources)?funnel.top_sources:[];$('#adminTrafficSources').innerHTML=sources.length?`<b>מקורות כניסה</b><div>${sources.map(row=>`<span>${esc(row.source||'ישיר')} <strong>${Number(row.visitors||0)}</strong></span>`).join('')}</div>`:'<small>מקורות כניסה יופיעו לאחר תחילת המדידה.</small>';
