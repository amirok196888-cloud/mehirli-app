...
$('#signupBtn').onclick=async()=>{trackAppEvent('trial_cta_click');const email=$('#authEmail').value.trim(),password=$('#authPassword').value,name=$('#authName').value.trim(),role='professional';if(!email||password.length<6){toast('הזן אימייל וסיסמה של לפחות 6 תווים');return}if(!$('#signupLegalConsent').checked){toast('כדי להירשם יש לאשר את תנאי השימוש ומדיניות הפרטיות');return}const {data,error}=await db.auth.signUp({email,password,options:{data:{role,full_name:name,legal_version:LEGAL_VERSION,legal_accepted_at:new Date().toISOString()}}});if(error){toast(error.message);return}const isNewSignup=!!data.user&&(!Array.isArray(data.user.identities)||data.user.identities.length>0);if(isNewSignup){await trackAppEvent('trial_signup');trackMetaTrialSignup(data.user)}if(data.session){state.user=data.user;await loadMe();trackAppEvent('trial_activated');await routeAfterLogin();toast('ההרשמה הושלמה — תקופת הניסיון הופעלה')}else{$('#authNote').textContent='נשלח אליך אימייל לאישור ההרשמה. לאחר האישור חזור והתחבר.'}};
...
function prepareJobQuotePdf(job){
  ...
  const promise=createQuotePdfFile(job).then(file=>{quotePdfCache.set(key,{status:'ready',file});const btn=$('#jobDetailContent')?.querySelector('[data-job-action="quote-pdf"]');if(btn&&state.selectedProJob?.id===job.id){btn.disabled=false;btn.textContent=job.quote_pdf_path?'📄 צור PDF תקין מחדש':'📄 צור ופתח PDF'}const waBtn=$('#jobDetailContent')?.querySelector('[data-job-action="quote-whatsapp"]');if(waBtn&&state.selectedProJob?.id===job.id){waBtn.disabled=false;waBtn.textContent='💬 שתף PDF ב־WhatsApp'}return file}).catch(error=>{quotePdfCache.delete(key);const btn=$('#jobDetailContent')?.querySelector('[data-job-action="quote-pdf"]');if(btn&&state.selectedProJob?.id===job.id){btn.disabled=false;btn.textContent='נסה שוב להכין PDF'}throw error});
...
async function renderProJobDetail(){
  ...
  $('#jobDetailContent').innerHTML=`<div class="quote-send-callout card"><span id="quoteStorageStatus" class="quote-send-status ${j.quote_pdf_path?'stored':''}">${j.quote_pdf_path?'ה־PDF שמור באפליקציה':'ההצעה נשמרה — ה־PDF ייווצר בזמן השליחה'}</span><h3>שליחת ההצעה ל־${esc(j.customer_name)}</h3><p>בלחיצה אחת ניצור קובץ PDF אמיתי ונפתח את תפריט השיתוף בטלפון. בחר WhatsApp ואת הלקוח ושלח את הקובץ.</p><button class="primary big whatsapp-action" data-job-action="quote-whatsapp" disabled>⏳ מכין PDF…</button></div>
...
async function loadProSettingsForm(){
  if(!requireServiceAccess())return;
  const s=await loadProSettings();$('#settingsTrade').value=s.trade;$('#settingsHourlyRate').value=s.default_hourly_rate;$('#settingsTravelCost').value=s.default_travel_cost;$('#settingsHalfDayPrice').value=s.half_day_price;$('#settingsFullDayPrice').value=s.full_day_price;$('#settingsHalfDayHours').value=s.half_day_hours;$('#settingsFullDayHours').value=s.full_day_hours;$('#settingsOvertimeRate').value=s.overtime_rate;$('#settingsTimeRounding').value=String(s.time_rounding_minutes||15);$('#settingsPaymentProvider').value=s.payment_provider||'';$('#settingsPaymentLink').value=s.payment_link||'';$('#settingsLicenseNumber').value=s.electrician_license_number||'';$('#settingsLicenseExpiry').value=s.electrician_license_expiry||'';$('#settingsQuoteTerms').value=s.quote_terms||'';$('#electricianSettings').classList.toggle('hidden',s.trade!=='electrician');renderPaymentConnectionStatus();show('#proSettingsView')
}
...
async function loadAdmin(){
  ...
  const [{data:summary,error:se},{data:jobs,error:je},{data:businesses,error:be},{data:billing,error:bse},{data:marketing,error:me}]=await Promise.all([
    db.rpc('admin_professional_summary_v33'),db.rpc('admin_list_pro_jobs'),db.rpc('admin_list_businesses_v33'),rpcWithFallback('admin_get_billing_settings_v38','admin_get_billing_settings_v33'),rpcWithFallback('admin_marketing_summary_v49','admin_marketing_summary_v40')
  ]);
  ...
  const funnel=marketing||{};$('#adminUniqueVisitors').textContent=funnel.unique_visitors_30d??funnel.unique_visitors??0;$('#adminVisitors30d').textContent='30 הימים האחרונים';$('#adminTrialClicks').textContent=funnel.trial_clicks||0;$('#adminTrialSignups').textContent=funnel.trial_signups||0;$('#adminAppInstalls').textContent=funnel.installs||0;$('#adminPayingCustomers').textContent=funnel.paying_customers||0;$('#adminVisitClickConversion').textContent=`${Number(funnel.visitor_to_click_percent||0).toLocaleString('he-IL')}%`;$('#adminClickTrialConversion').textContent=`${Number(funnel.click_to_trial_percent||0).toLocaleString('he-IL')}%`;$('#adminTrialPaidConversion').textContent=`${Number(funnel.trial_to_paid_percent||0).toLocaleString('he-IL')}%`;
